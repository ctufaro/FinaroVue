<template>
    <div>        
        <v-tabs fixed-tabs light slider-color="#EF4139">
            ...
            <v-tab to="/trendfad" style="display:none;">Fad Chart</v-tab>
        ...
        <div>            
            <div class="btn-group w-100 searchcolumn-filters" role="group" aria-label="Basic example">
                <button type="button" @click.prevent="openDialog=true; buysell='buy'" class="btn btn-outline-success btn-lg btn-rnd">BUY</button>
                <div style="width:80px;"></div>
                <button type="button" @click.prevent="openDialog=true; buysell='sell'" class="btn btn-outline-danger btn-lg btn-rnd">SELL</button>
            </div>
        </div> 
        <OrderForm :visible="openDialog" @close="openDialog=false" :buysell="buysell"/>       
    </div>
</template>
